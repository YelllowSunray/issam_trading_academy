//+------------------------------------------------------------------+
//| JournalSyncEA.mq5  (v4 — Next.js API + multi-account)              |
//| Stuurt gesloten trades + heartbeat naar de Next.js journal API.    |
//|                                                                      |
//| Installatie:                                                        |
...
void SendPositionTrade(long posId){
   ...
   HistorySelect(BackfillFrom, TimeCurrent());
...
void PostJson(string url, string json){
   ...
   if(StringLen(IngestSecret) > 0){
      headers += "x-mt5-secret: " + IngestSecret + "\r\n";
   }
   ResetLastError();
