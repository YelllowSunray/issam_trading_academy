...
export async function GET(req: Request) {
  return withApiError(async () => {
    ...
    if (!tradeId) return jsonError(await tRequest("api.tradeIdRequired"));
...
export async function POST(req: Request) {
  return withApiError(async () => {
    const user = await requireAuthUser(req);
    if (!groqConfigured()) return jsonError(await tRequest("api.groqMissing"), 500);
    const uid = await resolveTargetUid(req, user);
    if (uid !== user.uid) return jsonError("forbidden", 403);
    const body = (await req.json()) as { tradeId?: string };
    if (!body.tradeId) return jsonError(await tRequest("api.tradeIdRequired"));
