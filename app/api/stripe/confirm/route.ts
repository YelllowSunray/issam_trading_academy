...
export async function POST(req: Request) {
  return withApiError(async () => {
    ...
    if (!sessionId) return jsonError(await tRequest("api.sessionIdRequired"));
    ...
    const uid =
      ...
      (typeof session.client_reference_id === "string"
        ...
        : null);
    if (uid !== user.uid) return jsonError(await tRequest("api.sessionMismatch"), 403);
    ...
    if (!paid && session.status !== "complete") {
      return jsonError(await tRequest("api.paymentPending"), 400);
    }
